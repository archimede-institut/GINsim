...
public class InteractionsPanel extends GsPanel implements ItemListener, ActionListener {
  ...
  class SigKey {
	...
	public InteractionsPanel(TBClientPanel p) {
		super();
		try {
			URL url = JFrame.class.getResource("/fr/univmrs/tagc/GINsim/resource/icon/in_on.png");
			in_on = new ImageIcon(url);
			url = JFrame.class.getResource("/fr/univmrs/tagc/GINsim/resource/icon/out_on.png");
			out_on = new ImageIcon(url);
			url = JFrame.class.getResource("/fr/univmrs/tagc/GINsim/resource/icon/show.png");
						...
						if (node.getChild(j).isSelected()) {
							...
							if (v.size() > 0) {
								...
								if (ns >= 100)
									...
									w = 2;
								clientPanel.applyEdgeStyle((RegulatoryMultiEdge)graph.getEdge(v1, v2), w);
							}
	...
	public void init( Graph graph, boolean resizeColumns) {
		...
		while (it.hasNext()) {
			...
			if (!((DTreeElementToggleButton)node).isSelected()) {
				Collection<RegulatoryMultiEdge> edges = graph.getOutgoingEdges(vertex);
				...
				Collection<RegulatoryMultiEdge> edges = graph.getIncomingEdges(vertex);
				for (RegulatoryMultiEdge edge: edges) {
					for (int k = 0; k < edge.getEdgeCount(); k++) {
