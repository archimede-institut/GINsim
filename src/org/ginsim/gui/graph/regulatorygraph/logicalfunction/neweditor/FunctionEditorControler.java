...
public class FunctionEditorControler {
	...
	public FunctionEditorControler(FunctionEditorEditPanel p1, FunctionEditorDisplayPanel p2) {
		editPanel = p1;
    ...
    switch(action) {
    	...
    	case MODIF_TERM_ADD :
    	case MODIF_TERM_REM :
    		interaction = (ListInteraction)((Vector)p).firstElement();
    	...
    	case SET_AND_MODE :
			...
			case CANCEL_EDIT :
				...
				break;
			case COMPACT :
				expression = (TreeExpression)p;
				params = expression.getChilds();
				v = new Vector();
				value = ((TreeValue)expression.getParent()).getValue();
				for (int i = 0; i < params.size(); i++) {
					lp = new LogicalParameter(value);
					lp.setEdges(((TreeParam) params.elementAt(i)).getEdgeIndexes());
					v.addElement(lp);
				}
				c = new FunctionsCreator(graph, v, vertex);
				...
				break;
			case USER :
				expression = (TreeExpression)((Vector)p).firstElement();
			...
			case QUINE_DNF :
				v = (Vector)p;
				expression = (TreeExpression)v.firstElement();
