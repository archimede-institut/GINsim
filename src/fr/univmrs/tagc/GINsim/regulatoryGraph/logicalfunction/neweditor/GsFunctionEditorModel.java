...
public class GsFunctionEditorModel {
  ...
  public GsFunctionEditorModel(GsFunctionEditorControler c) {
  ...
  public void init(GsTreeInteractionsModel m, GsFunctionPanel p) {
    ...
    selectedArea = (Point)controler.exec(GsFunctionEditorControler.GET_TERM, v);
  ...
  public void reset() {
		if (selectedArea.x != -1) {
			String s = (String) controler.exec(GsFunctionEditorControler.GET_SELECTED_STRING, selectedArea);
			String s0 = functionPanel.getCurrentText();
	...
	private Vector getInteractionList(String s) {
		...
		for (int i = 0; i < t.length; i++) {
			if (!t[i].equals("")) {
				...
				if (found)
					le.addElement(new GsListInteraction(ed, k, not));
				else {
					le = null;
					break;
				}
  ...
  public GsFunctionPanel getFunctionPanel() {
  ...
  public void setAndMode(boolean b) {
	...
	public void addOpTerm(boolean and, Vector interactions, String operator, boolean par, boolean not) {
		...
		for (int i = 0; i < interactions.size(); i++) {
			interaction = (GsListInteraction)interactions.elementAt(i);
