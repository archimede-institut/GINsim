...
public class GsFunctionEditorDisplayPanel extends GsPanel implements ItemListener, ActionListener {
	...
	public GsFunctionEditorDisplayPanel() {
		super();
		initGraphic();
		initListeners();
	}
	...
	public void init(GsTreeExpression e) {
		expression = e;
	...
	public void itemStateChanged(ItemEvent e) {
		...
		if (e.getSource() == quineRadioButton) {
			...
			if (userInputRadioButton.isSelected()) {
				...
				controler.exec(GsFunctionEditorControler.USER, v);
			}
		}
		else if (e.getSource() == compactRadioButton) {
			if (compactRadioButton.isSelected())
				controler.exec(GsFunctionEditorControler.COMPACT, expression);
		}
		else if (e.getSource() == rawDNFRadioButton) {
			if (rawDNFRadioButton.isSelected())
				controler.exec(GsFunctionEditorControler.DNF, expression);
		}
			...
			if (quineDNF.isSelected()) {
				...
				controler.exec(GsFunctionEditorControler.QUINE_DNF, v);
			}
		}
			...
			if (quineCNF.isSelected()) {
				...
				v.addElement(cancelQuineButton);
				v.addElement(displayProgressBar);
				controler.exec(GsFunctionEditorControler.QUINE_CNF, v);
			}
		}
		else if (e.getSource() == cancelQuineButton) {
			controler.exec(GsFunctionEditorControler.QUINE_DNF, null);
		}
	}
	public void cancel() {
		controler.exec(GsFunctionEditorControler.CANCEL_DISPLAY, expression);
	}
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == okButton) {
			controler.exec(GsFunctionEditorControler.VALIDATE_DISPLAY, expression);
		}
