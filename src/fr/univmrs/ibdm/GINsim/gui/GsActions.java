...
/**
 * this class creates the menubar, the toolbar and the main actions for GINsim GUI. 
 */
public class GsActions  implements GraphChangeListener {
	...
	private void initialize()	{
		...
		actionSaveAs = new GsBaseAction("STR_saveAs",
										null,
										"STR_saveAs_descr",
		...
		actionSaveSubGraph = new GsBaseAction("STR_saveSubGraph",
										null,
										"STR_saveSubGraph_descr",
										null) {
		...
		actionMergeGraph = new GsBaseAction("STR_mergeGraph",
										null,
										"STR_mergeGraph_descr",
										null) {
		...
		actionCopy = new GsBaseAction("STR_copy",
										null,
										"STR_copy_descr",
		...
		actionPaste = new GsBaseAction("STR_paste",
										null,
										"STR_paste_descr",
		...
		actionSelectAll = new GsBaseAction("STR_selectAll",
											null,
											"STR_selectAll_descr",
		...
		actionInvertSelection = new GsBaseAction("STR_invertSelection",
												null,
												"STR_invertSelection_descr",
												null) {
		...
		actionUndo = new GsBaseAction("STR_undo",
						GsEnv.getIcon("undo.gif"),
						"STR_undo_descr",
						null) {
		...
		actionRedo = new GsBaseAction("STR_redo",
				GsEnv.getIcon("redo.gif"),
				"STR_redo_descr",
				null) {
		...
		actionEditDefault = new GsEditSwitchAction("STR_edit", GsEnv.getIcon("editmode.gif"), "STR_edit_descr", 
                KeyStroke.getKeyStroke( KeyEvent.VK_ESCAPE, InputEvent.CTRL_MASK), this, GsActions.MODE_DEFAULT, 0);
		mi_edit = new JMenuItem(actionEditDefault);
        
		actionZoomIn = new GsBaseAction("STR_zoomIn",
											null,
											"STR_zoomIn_descr",
		...
		actionZoomOut = new GsBaseAction("STR_zoomOut",
											null,
											"STR_zoomOut_descr",
		...
		actionNormalSize = new GsBaseAction("STR_Normalsize",
											null,
											"STR_NormalSize_descr",
		...
		actionVertexToFront = new GsBaseAction("STR_vertextofront",
						null,
						"STR_vertextofront_descr",
						null) {
		      private static final long serialVersionUID = 1884358515751192901L;

		      public void actionPerformed(java.awt.event.ActionEvent e) {    
		          viewcallback.vertexToFront(((JCheckBoxMenuItem)e.getSource()).getState()); 
		      }
		  };
				
		actionDisplayEdgeName = new GsBaseAction("STR_displayEdgeName",
												null,
												"STR_displayEdgeName_descr",
												null) {
					private static final long serialVersionUID = 2417319229337107021L;

					public void actionPerformed(java.awt.event.ActionEvent e) {    
						viewcallback.displayEdgeName(((JCheckBoxMenuItem)e.getSource()).getState()); 
					}
		};
				
				
		actionDivideWindow = new GsBaseAction("STR_DivideWindow",
								null,
								"STR_DivideWindow_descr",
								null) {
	    ...
	    actionDisplayMiniMap = new GsBaseAction("STR_DisplayMiniMap",
												null,
												"STR_DisplayMiniMap_descr",
												null) {
		...
		if (false) {
			...
			for (int i=0 ; i<v_graph.size() ; i++) {
				...
				openMenu.add(new GsOpenAction(gd, GsOpenAction.MODE_OPEN, mainFrame));
				if (gd.canCreate()) {
					newMenu.add(new GsOpenAction(gd, GsOpenAction.MODE_NEW, mainFrame));
				}
			}
            ...
            actionNew = new GsOpenAction(gd, GsOpenAction.MODE_NEW, mainFrame, "STR_new", "STR_new_descr", KeyStroke.getKeyStroke( KeyEvent.VK_N, ActionEvent.CTRL_MASK ));
            fileMenu.add(actionNew);
            actionOpen = new GsOpenAction(gd, GsOpenAction.MODE_OPEN, mainFrame, "STR_open", "STR_open_descr", KeyStroke.getKeyStroke( KeyEvent.VK_O, ActionEvent.CTRL_MASK ));
    ...
    public void updateRecentMenu() {
        ...
        Vector v_recent = GsOptions.getRecent();
        GsGraphDescriptor gd = GsGinsimGraphDescriptor.getInstance();
        for (int i=v_recent.size()-1 ; i>=0 ; i--) {
            recentMenu.add(new GsOpenAction(gd, mainFrame, (String)v_recent.get(i)));
        }
        if (v_recent.size() == 0) {
    ...
    public void setDefaults() {
        btt_divideWindow.setState(((Boolean)GsOptions.getOption("display.dividewindow", Boolean.FALSE)).booleanValue());
        actionDivideWindow.actionPerformed(new ActionEvent(btt_divideWindow,0, ""));
        
        btt_displayGrid.setState(((Boolean)GsOptions.getOption("display.grid", Boolean.TRUE)).booleanValue());
        actionDisplayGrid.actionPerformed(new ActionEvent(btt_displayGrid,0, ""));
        
        btt_gridActive.setState(((Boolean)GsOptions.getOption("display.gridactive", Boolean.FALSE)).booleanValue());
    ...
    public void graphChanged(GsNewGraphEvent event) {
        ...
        if (v_modes != null) {
            ...
            for (int i=0 ; i<v_modes.size() ; i++) {
                GsEditSwitchAction sa = new GsEditSwitchAction((GsEditModeDescriptor)v_modes.get(i), this);
		...
		addToMenu(exportMenu, graph.getExport(), GsActionProvider.ACTION_EXPORT, graph);
		addToMenu(exportMenu, graph.getSpecificExport(), GsActionProvider.ACTION_EXPORT, graph);
		addToMenu(exportMenu, graphManager.getExport(), GsActionProvider.ACTION_EXPORT, graph);
		
		// fill layout menu
		layoutMenu.removeAll();
		addToMenu(layoutMenu, graph.getLayout(), GsActionProvider.ACTION_LAYOUT, graph);
		addToMenu(layoutMenu, graph.getSpecificLayout(), GsActionProvider.ACTION_LAYOUT, graph);
		addToMenu(layoutMenu, graphManager.getLayout(), GsActionProvider.ACTION_LAYOUT, graph);
		
		// fill action menu
		actionMenu.removeAll();
		actionMenu.add(layoutMenu);
		addToMenu(actionMenu, graph.getAction(), GsActionProvider.ACTION_ACTION, graph);
		addToMenu(actionMenu, graph.getSpecificAction(), GsActionProvider.ACTION_ACTION, graph);
		addToMenu(actionMenu, graphManager.getAction(), GsActionProvider.ACTION_ACTION, graph);
		...
		if (v_actions != null) {
			for (int i=0 ; i<v_actions.size() ; i++) {
				...
				if (obj instanceof GsPluggableActionDescriptor) {
					menu.add( new GsPluggableAction((GsPluggableActionDescriptor)obj, mainFrame) );
				    ...
				    if (t_action != null) {
				        for (int j=0 ; j<t_action.length ; j++) {
				            menu.add( new GsPluggableAction(t_action[j], mainFrame) );
				        }
				    }
				}
			}
    ...
    /**
     * choose the new mode and submode.
     * 
     * the submode is an option that will be passed to newly created vertices/edges
     * (different kinds of fully graph dependant objects just one click away).
     * 
     * @param mode the new editing mode: should be on of MODE_DEFAULT, MODE_ADD_VERTEX, MODE_ADD_EDGE or MODE_ADD_EDGE_POINT/
     * @param submode the submode, ie option for the object.
     * @param locked if true, the mode won't change when creating the next object.
     */
    ...
    /**
     * change the editing mode if it's not locked.
     * (according to last call to <code>setCurrentMode(int, int, boolean)</code>) 
     */
