...
public class GsFunctionEditorModel {
  ...
  public void deleteCurrentTerm() {
    ...
    if (currentTerm == terms.size()) currentTerm--;
    if (currentTerm == 0) {
      getCurrentTerm().operator = 0;
      getCurrentTerm().update();
    }
    if (terms.isEmpty()) addTerm(null, false, 0);
  }
  ...
  private Point getPosition(int index) {
    ...
    if (j0 < s0.length()) while (s0.charAt(j0) == ' ') {
      j0++;
      if (j0 >= s0.length()) break;
    }
    if (((j - 1) < s0.length()) && ((j - 1) >= 0)) while (s0.charAt(j - 1) == ' ') j--;
    if (n == index) j++;
