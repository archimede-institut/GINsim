...
/**
 * Main class of the animator plugin. use the <code>animate</code> static
 * method to run the animation on a dynamic or regulatory graph: It performs
 * some checks and run the GUI once everything looks OK.
 */
public class GsRegulatoryAnimator extends AbstractListModel implements
		GraphChangeListener {
	...
	private static final Color[] BGCOLS = { Color.WHITE,
	 ...
	 * @param dynGraph
	 */
	private GsRegulatoryAnimator(JFrame frame, GsRegulatoryGraph regGraph,
	...
	private void initAnim() {
		...
		while (it.hasNext()) {
			...
			for (int j = 0; j < l_edge.size(); j++) {
				GsRegulatoryMultiEdge edge = (GsRegulatoryMultiEdge) ((GsDirectedEdge) l_edge
						.get(j)).getUserObject();
				ereader.setEdge(l_edge.get(j));
				oldColors.put(edge, new StoreColor(ereader));
			}
		}
		dynGraph.getGraphManager().getEventDispatcher()
				.addGraphChangedListener(this);
	...
	protected void endAnim() {
		...
		dynGraph.getGraphManager().getEventDispatcher()
				.removeGraphChangeListener(this);
		revertPath(0);
	...
	protected void add2path(Object vertex) {
		...
		if (path.size() > 0
				&& dynGraph.getGraphManager().getEdge(
						path.get(path.size() - 1), vertex) == null) {
			return;
		}
		path.add(vertex);
		unMarkAllPath();
	...
	private void markAllPath() {
		...
		Iterator it = dynGraph.getGraphManager().getOutgoingEdges(
				path.get(path.size() - 1)).iterator();
	...
	/**
	 * colorize the regulatory graph according to a given state (node of the
	 * dynamic graph).
	 * 
	 * @param state
	 */
	...
	/**
	 * rewind the path. basically revert all color changes made on the dyngraph
	 * and eventually apply back the first ones!
	 * 
	 * @param index
	 *            index of the last vertex to remove.
	 */
	 ...
	 * 
	 * @param i
	 *            index of the selected element in the path list
	 */
