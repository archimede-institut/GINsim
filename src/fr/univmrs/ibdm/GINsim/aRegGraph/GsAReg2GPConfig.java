...
/**
 * Export a path in the state transition graph to gnuplot scripts. This class
 * offers static methods to do the job, and an UI to configure it
 */
public class GsAReg2GPConfig extends JDialog {
	...
	private JTable getBlockTable() {
		if (blockTable == null) {
			...
			blockTable.setModel(new GsAReg2GPModel(nodeOrder, t_selected));
		}
		return blockTable;
	}
	...
	protected void export() {
		...
		try {
			gnuplotsFilename = GsOpenAction.selectSaveFile(null, null, null,
					".gnuplot");
		} catch (GsException e) {
			GsEnv.error(e, null);
			return;
		...
		File datafile = new File(gnuplotsFilename.substring(0, gnuplotsFilename
				.length() - 8)
				+ ".data");
		switch (combo_choice.getSelectedIndex()) {
		case 0:
			doSaveGnuPlotArrowPath(scriptfile, datafile, path, nodeOrder,
					t_selected);
			break;
		case 1:
			doSaveGnuPlotMultiPath(scriptfile, datafile, path, nodeOrder,
					t_selected);
			break;
		}
		dispose();
	 ...
	 * @param selected
	 */
	public static void doSaveGnuPlotArrowPath(File scriptfile, File datafile,
			List path, Vector nodeOrder, boolean[] selected) {
		...
		try {
			...
			for (i = 1; i < nbExported + 1; i++) {
				out.write("set style arrow " + i + " filled linetype " + i
						+ " size screen 0.025,30.000,45.000\n");
			}
			out.write("\n");
			...
			for (i = 0; i < nbExported - 1; i++) {
				out.write(s_plot + "1:" + (2 * i + 2) + ":(0):" + (2 * i + 3)
						+ " with vectors arrowstyle " + (i + 1) + " title '"
						+ nodeOrder.get(exported[i]) + "',\\\n");
			}
			out.write(s_plot + "1:" + (2 * i + 2) + ":(0):" + (2 * i + 3)
					+ " with vectors arrowstyle " + (i + 1) + " title '"
					+ nodeOrder.get(exported[i]) + "'\n");
	 ...
	 * @param selected
	 */
	public static void doSaveGnuPlotMultiPath(File scriptfile, File datafile,
			List path, Vector nodeOrder, boolean[] selected) {
		int[] exported = new int[selected.length];
		int nbExported = 0;
		for (int i = 0; i < selected.length; i++) {
		...
		try {
			...
			out.write("set origin 0, 0; " + s_plot + "2 title '"
					+ nodeOrder.get(exported[0]) + "' with lines\n");
			// change parameters for following plots
			out.write("set xtics -1,1,-1\n"); // set noxtics doesn't seems to
												// work, work arround it..
			out.write("set xlabel \"\"\n");

			// then add other plots
			for (i = 1; i < nbExported; i++) {
				out.write("set origin 0, " + (i * yscale) + "; " + s_plot
						+ (i + 2) + " title '" + nodeOrder.get(exported[i])
						+ "' with lines\n");
			}
