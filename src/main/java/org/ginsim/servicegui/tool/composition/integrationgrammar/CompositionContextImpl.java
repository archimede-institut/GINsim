...
return dialog.getGraph().getNodeOrder();
	}

	@Override
	public List<NodeInfo> getLowLevelComponents() {
		return dialog.getGraph().getNodeInfos();
	}

	@Override
	public Set<Integer> getNeighbourIndices(int instance, int distance) {
		Set<Integer> results = this.getAllNeighbourIndicesCloserThan(instance, distance);
		Set<Integer> frontier = this.getAllNeighbourIndicesCloserThan(instance, distance-1);
		
		results.removeAll(frontier);
		return results;
	}
	
	private Set<Integer> getAllNeighbourIndicesCloserThan(int instance, int distance){
