...
/**
 * 
 * Writer class for common.lnt including general functions and type definitions,
 * based on the maximum value of the multi-valued components, if any are
 * referred to in the model specification
 * 
	...
	public String toString() {
		...
		if (hasMulti) {
			for (int i = 0; i <= maxValue; i++) {
				output += "function " + i + "M : Multi is return Multi (" + i
						+ ") end function\n";
			}
		}
		...
		for (int i = 0; i < operations.length; i++) {
			output += "function _" + operations[i]
					+ "_ (op1 : Binary, op2 : Nat) : Binary is\n";
		...
		if (hasMulti) {
			for (int i = 0; i < operations.length; i++) {
				output += "function _" + operations[i]
						+ "_ (op1 : Multi, op2 : Nat) : Multi is\n";
				output += "\treturn Multi (Nat (op1) " + operations[i]
						+ " op2)\n";
				output += "end function\n";
		...
		if (hasMulti) {
			...
			output += "\t\tselect\n";
			output += "\t\t\tif u !=" + maxValue
					+ "M then Val := u+1 else stop end if\n";
		...
		output += "process Stable_State [" + CADPWriter.getStableActionName()
				+ ":None] (b:Bool) is\n";
		output += "\tif b then " + CADPWriter.getStableActionName()
				+ " end if\n";
		output += "end process\n";
