...
public class IntegrationFunctionMapping {
	...
	public void addMapping(RegulatoryNode input, List<RegulatoryNode> properList, IntegrationFunction integrationFunction) throws GsException{
		if(!input.isInput()){
			throw new GsException(GsException.GRAVITY_ERROR,"Only input components can be mapped");
		}
		Iterator<RegulatoryNode> iterator = properList.iterator();
		while(iterator.hasNext()){
			if(iterator.next().isInput()){
				throw new GsException(GsException.GRAVITY_ERROR,"Only proper components can be arguments of integration functions");
			}
		}

		RegulatoryIntegration regulatoryIntegration = new RegulatoryIntegration(integrationFunction, properList);
