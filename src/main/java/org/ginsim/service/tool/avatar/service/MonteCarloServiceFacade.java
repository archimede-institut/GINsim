 ...
 * Instantiates a Monte Carlo simulation from arguments within a command line,
...
public class MonteCarloServiceFacade implements Service {
	...
	public static Simulation getSimulation(String[] args) throws Exception {
		...
		// if(args.length<2) throw new AvaException("Usage: model runs [random_init
		// [max_steps]]");
		String filename = AvaOptions.getStringValue("input", args);
		if (filename != null)
			sim = (MonteCarloSimulation) addModel(sim, filename);
