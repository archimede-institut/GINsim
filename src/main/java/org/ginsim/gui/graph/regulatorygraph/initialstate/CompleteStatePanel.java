...
public class CompleteStatePanel extends InitialStatePanel {
	...
	public CompleteStatePanel(NamedStateList states, NamedStateList istates, NamedStateList oracles, boolean several) {
        ...
        doLayout(states.getNodeOrder().size()>0,istates.getNodeOrder().size()>0, oracles==null ? false : oracles.size()>0);
	}
    ...
    public CompleteStatePanel clone(AvatarStateStore store){
    	AvatarStateStore clonedstore = new AvatarStateStore(store.nodes,store.inodes,store.allnodes,initPanel.getStateList(),inputPanel.getStateList(),oraclesPanel.getStateList());
    	CompleteStatePanel clone = new CompleteStatePanel(clonedstore.nstates,clonedstore.instates,clonedstore.oracles,true);
